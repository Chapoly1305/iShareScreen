...
class AudioSink:
    ...
    def __init__(
        ...
        # Peak amplitude over the session, measured in feed() on the
        # RX thread (not the RT callback), so it adds zero cost to
        # PortAudio's deadline-sensitive path. Lets us tell "Apple
        # sent decoded-near-zero filler" from "Apple sent real sound."
        self._peak = 0.0
    ...
    def feed(self, pcm: np.ndarray) -> None:
        ...
        # Apple's PT=101 stream sends decoded-to-zero PCM whenever the
        # host has nothing playing — every sample exact 0.0, ~50 pkt/s.
        # Skip silent chunks rather than burn the queue + PortAudio
        # callback playing literal zeros; output is bit-identical to
        # the underflow-fill path the callback already runs when the
        # queue is empty.
        if not pcm.any():
            return

        chunk_peak = float(np.abs(pcm).max())
        now = time.monotonic()
        with self._lock:
    ...
    def stop(self) -> None:
        ...
        log.info(
            "audio sink stopped: %d feeds, %d frames played "
            "(%d underflow %.1f%%, peak=%.1f dBFS)",
            feed, played, silence, ratio * 100.0, peak_db,
        )
