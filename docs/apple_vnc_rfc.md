...
- **`0x1c` per-stream offer-blob internals** (handed to AVConference) and the server *answer* layout — top-level offer framing (including the `+0x06` `flags` `u32` and the `+0x0a`/`+0x0c`/`+0x0e` offer-length words) is recovered (§10.3), and the audio blob's field-4 requested-bitrate (`preferredMediaBitRate`) is now mapped (§10.3); the **video** offer-blob internals and the full answer layout remain the gap.
