...
- **Direction**: server→client. **Transport**: framebuffer-update rectangle (`x = y = w = h = 0` for metadata variants). **Receiver behavior: render** (cursor pixmap). Advertised but not emitted in the capture — payload is a **revision gap** pending a session that carries it.
- Encoder `EncodeCursorImageWithAlpha` (`0x100019aa8`). The rectangle header carries the cursor geometry (`x` = hotspot-x, `y` = hotspot-y, `w`/`h` = cursor size, per the RFB cursor convention); the payload is `u32 cache_id || u32 compressed_len || zlib_payload`. `compressed_len = 0` signals **cache reuse** (no pixels; new cache ids count up from `0x3e8`). The zlib stream (`deflate` level 9, `Z_SYNC_FLUSH`) carries the cursor's **alpha plane separated from its 24-bit RGB** (source is 32-bit BGRA; not interleaved RGBA).
...
u16   version         (= 0x0001)
u32   selected_screen (observed 0)
u16   x
u16   y
u16   w
u16   h
...
Switches the server to server-driven framebuffer streaming. Observed `x,y,w,h` = the logical geometry. `HandleAutoFrameBufferUpdateMessage`: `version` is the incremental/update flag; `selected_screen` is the target screen id, with `0xffffffff` as the sentinel for "all/main displays" (it clears the per-screen-selection bool). After sending this, a client SHOULD NOT continue to poll with `FramebufferUpdateRequest`.
...
16. server metadata burst (`0x451`, `0x453`, `0x455`, `0x456`, `0x450`) and steady state
